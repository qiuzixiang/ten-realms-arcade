...
import { V4_PROGRESS_KEY, readStoredJson, tutorialStorageKey, writeStoredJson, writeStoredValue, readStoredValue } from "./storage.mjs";
...
// Games can be opened from a deep link without visiting the V4.0 guide first.
// Register from this shared bootstrap so those entries still receive V4 offline support.
...
if (config) {
  ...
  let progress = normalizeProgress(readStoredJson(V4_PROGRESS_KEY, null));
  let cardIndex = 0;
  ...
  function buildDock() {
    ...
    node.setAttribute("aria-label", "4.0 探索进度与教程");
    applyAccent(node);
    node.innerHTML = `
      <div><small>FOURTH EXPEDITION · LOCAL</small><button type="button" data-rank>新境访客</button></div>
  ...
  function buildRewards() {
    ...
    node.setAttribute("aria-labelledby", "v4-progress-title");
    applyAccent(node);
    node.innerHTML = `
      <div class="realm-progress-dialog__shell">
        <header><div><small>FOURTH EXPEDITION · REWARD MAP</small><h2 id="v4-progress-title">4.0 成长图鉴</h2></div><button type="button" data-progress-close aria-label="关闭成长图鉴">×</button></header>
        ...
        <section><div class="realm-progress-dialog__heading"><h3>如何获得 XP</h3><span>4.0 十款共享</span></div>
  ...
  function buildGuide() {
    ...
    node.setAttribute("aria-labelledby", "v4-guide-title");
    applyAccent(node);
    node.innerHTML = `
      <div class="realm-guide-dialog__shell">
        <header><div><small>HOW TO PLAY · ${tutorial.title}</small><h2 id="v4-guide-title"></h2></div><button type="button" data-close aria-label="跳过教程">跳过</button></header>
  ...
  function renderCard() {
    ...
    guide.querySelector("#v4-guide-title").textContent = card.title;
  ...
  function complete(payload = {}) {
    progress = mergeProgress(progress, normalizeProgress(readStoredJson(V4_PROGRESS_KEY, null)));
    const result = awardCompletion(progress, { ...payload, realm: realmId });
    if (!result.accepted) throw new TypeError("V4.0 completion requires one stable, canonical event identity.");
    if (!writeStoredJson(V4_PROGRESS_KEY, result.progress)) {
      throw new Error("Unable to persist V4.0 shared progress.");
    }
  ...
  window.addEventListener("storage", (event) => {
    if (event.key !== V4_PROGRESS_KEY) return;
    progress = mergeProgress(progress, normalizeProgress(readStoredJson(V4_PROGRESS_KEY, null)));
    updateDock();
    updateRewards();
  });
  const host = Object.freeze({ realm: realmId, complete, getSnapshot: snapshot, openTutorial: () => openTutorial(false) });
  window.RealmArcade = host;
  window.TenRealmsV4 = host;
  ...
  window.dispatchEvent(new CustomEvent("ten-realms-v4:realm-ready", { detail: { realm: realmId } }));
