...
test("存档前缀只属于时砂邮路局", () => {
  strictEqual(STORAGE_PREFIX, "ten-realms-v2:games:time-sand-post:");
...
test("TenRealmsV2 是优先且可确认的完成通道", () => {
  const payload = completionFor();
  const calls = [];
  const target = {
    TenRealmsV2: { complete: (value) => calls.push(["native-v2", value.eventId]) },
    RealmArcade: { complete: (value) => calls.push(["realm-arcade", value.eventId]) },
  };
  equal(deliverCompletion(target, payload), { retained: true, confirmed: true, transport: "native-v2" });
  equal(calls, [["native-v2", payload.eventId]]);
  strictEqual(completionTransport(target, payload.eventId), "native-v2");
});
...
test("HTML 保持 v2.5 返回路径、共享层顺序与原生教程入口", () => {
...
test("CSS 保证无横滚、44px 触控、320/340 窄屏与减少动态", () => {
  ...
  match(css, /\.tutorial-visual\s*\{[^}]*aspect-ratio:\s*430\s*\/\s*520;[^}]*place-items:\s*center start/);
  match(css, /\.tutorial-visual img\s*\{[^}]*width:\s*auto;[^}]*height:\s*100%;[^}]*max-width:\s*none;[^}]*max-height:\s*none;[^}]*object-position:\s*left center/);
