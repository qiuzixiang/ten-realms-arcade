...
export const COMPLETION_SCHEMA = "ten-realms-v2.game-complete";
...
export function deliverCompletion(target, value) {
  ...
  if (["native-v2", "realm-arcade"].includes(previous)) {
    return Object.freeze({ retained: true, confirmed: true, transport: previous });
  }

  let transport = null;
  for (const [name, owner] of [["native-v2", target.TenRealmsV2], ["realm-arcade", target.RealmArcade]]) {
    try {
