...
import { V3_PROGRESS_KEY, readStoredJson, tutorialStorageKey, writeStoredJson, writeStoredValue, readStoredValue } from "./storage.mjs";
...
// Games can be opened from a deep link without visiting the V3.0 guide first.
// Register from this shared bootstrap so those entries still receive V3 offline support.
...
if (config) {
  ...
  let progress = normalizeProgress(readStoredJson(V3_PROGRESS_KEY, null));
  let cardIndex = 0;
  ...
  function buildDock() {
    ...
    node.setAttribute("aria-label", "3.0 探索进度与教程");
    applyAccent(node);
    node.innerHTML = `
      <div><small>THIRD EXPEDITION · LOCAL</small><button type="button" data-rank>新境访客</button></div>
  ...
  function buildRewards() {
    ...
    node.setAttribute("aria-labelledby", "v3-progress-title");
    applyAccent(node);
    node.innerHTML = `
      <div class="realm-progress-dialog__shell">
        <header><div><small>THIRD EXPEDITION · REWARD MAP</small><h2 id="v3-progress-title">3.0 成长图鉴</h2></div><button type="button" data-progress-close aria-label="关闭成长图鉴">×</button></header>
        ...
        <section><div class="realm-progress-dialog__heading"><h3>如何获得 XP</h3><span>3.0 十款共享</span></div>
  ...
  function buildGuide() {
    ...
    node.setAttribute("aria-labelledby", "v3-guide-title");
    applyAccent(node);
    node.innerHTML = `
      <div class="realm-guide-dialog__shell">
        <header><div><small>HOW TO PLAY · ${tutorial.title}</small><h2 id="v3-guide-title"></h2></div><button type="button" data-close aria-label="跳过教程">跳过</button></header>
  ...
  function renderCard() {
    ...
    guide.querySelector("#v3-guide-title").textContent = card.title;
  ...
  function complete(payload = {}) {
    progress = mergeProgress(progress, normalizeProgress(readStoredJson(V3_PROGRESS_KEY, null)));
    const result = awardCompletion(progress, { ...payload, realm: realmId });
    if (!result.accepted) throw new TypeError("V3.0 completion requires one stable, canonical event identity.");
    if (!writeStoredJson(V3_PROGRESS_KEY, result.progress)) {
      throw new Error("Unable to persist V3.0 shared progress.");
    }
  ...
  window.addEventListener("storage", (event) => {
    if (event.key !== V3_PROGRESS_KEY) return;
    progress = mergeProgress(progress, normalizeProgress(readStoredJson(V3_PROGRESS_KEY, null)));
    updateDock();
    updateRewards();
  });
  ...
  window.dispatchEvent(new CustomEvent("ten-realms-v3:realm-ready", { detail: { realm: realmId } }));
