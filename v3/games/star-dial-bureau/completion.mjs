...
function freezePayload(value) {
  return Object.freeze({ ...value, history: Object.freeze(value.history.map((move) => Object.freeze({ ...move }))), rewardIds: Object.freeze([...value.rewardIds]) });
}
...
export function normalizeCompletion(value) {
  ...
  const replay = level ? replayMoves(level.initialBoard, value.history) : null;
  const completedAt = canonicalTimestamp(value.completedAt);
  if (!level || !replay || !evaluateBoard(replay.board).complete || value.difficulty !== level.difficulty
...
/**
 * Locally settle the verified replay before notifying the shared V3 host.  A
 * stable event ledger gives this operation exactly-once semantics per run even
 * if a page is refreshed between the local write and the host delivery.
 */
export function settleCompletion({ profile, level, state, runId, elapsedMs, completedAt = new Date().toISOString() }) {
  ...
  if (!official || official !== level || !cleanProfile || !canonicalState || !canonicalState.complete
    ...
    throw new TypeError("A canonical solved state, official level, run ID and elapsed time are required.");
  }
  const eventId = `${GAME_ID}:${runId}:complete`;
  const prior = cleanProfile.settledEvents[eventId];
  if (prior) {
  ...
  if (!completedBefore) offer(reward(`${GAME_ID}:clear:${official.id}`, "clear", `${official.title} · 首次校准`, official.id, stamp));
  if (bestBefore === null || moves < bestBefore) offer(reward(`${GAME_ID}:best:${official.id}:${moves}`, "best", `${official.title} · 新校准记录 ${moves} 次`, official.id, stamp));
  if (moves <= official.par) offer(reward(`${GAME_ID}:reference:${official.id}`, "reference", `${official.title} · 达到可复算参考线`, official.id, stamp));
  ...
  if (!canonicalProfile) throw new TypeError("Canonical profile generation failed.");
...
export function deliverCompletion(target, payload) {
  ...
  try {
    ...
    try {
      ...
      // The host call is canonical. A convenience event cannot make it fail.
    }
...
export function queueCompletion(target, payload) {
  ...
  try {
    const existing = Array.isArray(target[COMPLETION_QUEUE]) ? target[COMPLETION_QUEUE].map(normalizeCompletion).filter(Boolean) : [];
