...
test("损坏会话仅清理本游戏会话键并安全回退", () => {
  const storage = new FakeStorage({
    ...
    "ten-realms-v3:games:foreign:session:v1": "keep-me",
  });
  const restored = loadSession(storage, getLevel);
  equal(restored.status, "invalid");
  equal(restored.session, null);
  equal(storage.getItem(STORAGE_KEYS.session), null);
  equal(storage.getItem("ten-realms-v3:games:foreign:session:v1"), "keep-me");
});
...
test("损坏 outbox 只移除本游戏 outbox", () => {
  const storage = new FakeStorage({
    ...
    "ten-realms-v3:games:other:completion-outbox:v1": "keep",
  });
  ...
  equal(storage.getItem("ten-realms-v3:games:other:completion-outbox:v1"), "keep");
});
...
test("教程入口统一缓存版本并提供跳过、看完与重看路径", async () => {
  ...
  match(app, /document\.activeElement !== document\.body[\s\S]*?elements\.tutorialButton/);
  match(app, /event\.key === " "[^\n]+event\.key === "Enter"/);
  match(app, /addEventListener\("realm:ready"[^\n]+flushCompletionOutbox/);
  match(app, /addEventListener\("ten-realms-v3:realm-ready"[^\n]+flushCompletionOutbox/);
...
test("页面具备 V3.0 接入、规范返回与脚本顺序", async () => {
...
test("生产代码只使用私有存储前缀且不执行全量清理", async () => {
  ...
  check(STORAGE_PREFIX.startsWith("ten-realms-v3:games:resonance-bell-room:"));
